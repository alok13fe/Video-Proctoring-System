...
export default function SignUp({ role, setPage, onClose}: SignUpProps){
  ...
  async function registerUser(e: React.FormEvent<HTMLFormElement>){
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/${role}/register`,
        formData
      );
  ...
  return (
    <main>
      <div className="min-h-screen w-screen sm:w-sm flex justify-center items-center">
        <div className="relative w-full h-screen sm:h-fit px-8 py-8 bg-white border shadow-md rounded-md">
          <span 
          ...
          <div className="text-center">
            ...
            <p className="text-muted-foreground mt-2">We&apos;re excited to have you join us</p>
          </div>
          <form className="py-4 space-y-3" onSubmit={registerUser}>
            ...
            {
              success &&
              <p className='mb-0 text-green-600 text-xs'>
            ...
            {
              error &&
              <p className='mb-0 text-red-600 text-xs'>
                {error}
              </p>
            }
            ...
            <button 
              type='submit'
              className={`w-full mt-3 py-2 bg-black text-white font-medium rounded-md ${loading ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'}`}
              disabled={loading}
            >
              Sign Up
            </button>
          ...
          <p className="mt-2 text-center text-muted-foreground text-sm">
            Already have an account? <button className="cursor-pointer" onClick={() => {setPage('signin')}}>Sign In</button>
          </p>
        </div>
      </div>
    </main>
